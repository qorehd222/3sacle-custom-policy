...
local stack_macro = require("custom.openssl.include.stack")
local stack_lib = require("custom.openssl.stack")
local asn1_lib = require("custom.openssl.asn1")
local digest_lib = require("custom.openssl.digest")
local extension_lib = require("custom.openssl.x509.extension")
local pkey_lib = require("custom.openssl.pkey")
local util = require "custom.openssl.util"
local txtnid2nid = require("custom.openssl.objects").txtnid2nid
...
local function digest(self, cfunc, typ)
  -- TODO: dedup the following with custom.openssl.digest
  local ctx
  if OPENSSL_11 then
...
function _M:get_serial_number()
  ...
  local lib = require("custom.openssl.bn")
...
function _M:get_pubkey()
  ...
  local lib = require("custom.openssl.pkey")
...
function _M:get_subject_name()
  ...
  local lib = require("custom.openssl.x509.name")
  -- the internal ptr is returned, ie we need to copy it
  return lib.dup(got)
end
...
function _M:get_subject_alt_name()
  ...
  local lib = require("custom.openssl.x509.altname")
  -- the internal ptr is returned, ie we need to copy it
  return lib.dup(got)
end
...
function _M:get_info_access()
  ...
  local lib = require("custom.openssl.x509.extension.info_access")
  -- the internal ptr is returned, ie we need to copy it
  return lib.dup(got)
end
...
function _M:get_crl_distribution_points()
  ...
  local lib = require("custom.openssl.x509.extension.dist_points")
  -- the internal ptr is returned, ie we need to copy it
  return lib.dup(got)
end
