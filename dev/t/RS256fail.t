...
=== TEST: Token Based Authentication fail (alg: RS256, base64 encoded secret)
            ...
            jwt_secret = {
                {
                    key = "service A",
                    algorithm = alg,
                    rsa_public_key = jwt_parser.base64_encode(rs256_public_key:gsub('QAB', 'zzz')),
                    consumer = {
...
=== TEST: Token Based Authentication fail (alg: RS256, non-base64 encoded secret)
--- http_config eval: $t::Util::HttpConfig
--- config
location = /t2 {
    default_type 'application/json';
            ...
            jwt_secret = {
                {
                    key = "service A",
                    algorithm = alg,
                    rsa_public_key = rs256_public_key:gsub('QAB', 'zzz'),
                    consumer = {
                        id = "68aca0ee-ca34-4fe8-8bb4-3657eaf7508c",
                        custom_id = "custom_id_1",
                        username ="David"
